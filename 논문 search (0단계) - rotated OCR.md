...
> **스코프:** 얼린 본 모델 앞에 **독립 앞단 모델(약 3B VLM)**을 두어 **회전된 문서의 OCR**을 살리는 것. 회전은 **임의 각도**를 목표로 하고, 본 모델의 vision encoder가 ViT 계열이 아니므로 **내부 feature에 의존하지 않는 설계**여야 한다.
...
**→ 아이디어 씨앗:**
1. **⚠️ 아키텍처는 차용할 수 없습니다.** 이 방법은 본 모델의 **ViT vision encoder에서 CLS 토큰을 뽑는 것**이 전제인데, 본 과제의 본 모델은 ViT 계열이 아니고 확장성도 필요합니다. **성능 참조점으로만** 쓰고, 앞단은 독립 모델로 갑니다. (학습 데이터도 비공개라 직접 구해야 합니다.)
...
3. **★ 저자가 future work로 "arbitrary-angle rotation"을 명시**했습니다. 12-class는 30° 단위라 그 사이 각도를 못 다뤄요. **임의 각도를 목표로 하는 우리에게는 이게 가장 명확한 빈틈**입니다 — 분류 프레임 자체의 한계라 클래스를 늘려도 해결되지 않습니다(각도 간 순서·거리 정보가 사라짐).
...
> **⚠️ 단, 이건 4~6방향 전제입니다.** 임의 각도로 가면 이진 지시함수로는 학습이 안 됩니다 — 37°를 40°로 맞힌 것과 130°로 맞힌 것이 똑같이 0이 되니까요. **각도 오차 기반 연속 reward로 바꿔야** 하고, 이건 ToolsRL이 zoom-in에 ModF1을 쓴 것과 같은 이유입니다.
