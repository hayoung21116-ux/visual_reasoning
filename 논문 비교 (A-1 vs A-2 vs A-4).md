...
**앞단 방식의 전이 가능성을 보여주는 선례입니다** — 다른 데이터셋·다른 과제·얼린 모델에 앞단만 갈아끼워 손실 대부분을 회복. (단 A-4는 feature tap 방식이라 아키텍처는 우리와 다릅니다.)
...
얼린 본 모델 + 독립 앞단이라는 구조가 그대로 맞습니다.

| | 우리와의 핏 |
|---|---|
| A-1 | ✗ 본 모델과 **함께 학습**해야 함. 우리는 못 함 |
| **A-2** | ✓ **얼린 대형 모델 + 독립 앞단**. SAM 실험이 정확히 그 세팅 |
| **A-4** | △ 얼린 본 모델 + 앞단이라는 **구조는 맞지만**, 본 모델 feature를 tap하므로 **ViT 비의존·확장성 요건과 충돌** |
| A-3 (RECON) | ✗ 범용 canonicalizer가 목표. 우리는 범용성 불필요 |

### 설계 확정 — 독립 앞단(A-2형)

이전에는 본 모델 feature 접근이 가능하므로 A-4형(feature tap)도 선택지였으나, **두 제약이 이를 배제합니다.**

- 본 모델의 vision encoder가 **ViT 계열이 아님** → A-4·Seeing Straight의 feature tap 설계를 옮길 수 없음
- **확장성 필요** → feature에 배선하면 본 모델을 바꿀 때마다 재학습

| | feature 사용 (A-4형) | **독립 (A-2형) ← 채택** |
|---|---|---|
| 앞단 크기 | 매우 작아도 됨 | **충분히 커야 함** |
| 본 모델 교체 | 재학습 필요 | **재사용 가능** |
| ViT 의존 | 있음 | **없음** |

**대가가 있습니다.** A-2의 SAM 실험에서 독립 canonicalizer는 **0.2M에서 1.9M으로 키워야** 원본 mAP가 유지됐습니다(62.34 → 59.28 → 62.13). 저자 설명은 *"표현력이 부족하면 복잡한 이미지를 identity로 매핑하지 못해서"*. 우리 앞단이 3B인 건 이 요구를 충족하고도 남지만, **"앞단이 약하면 멀쩡한 이미지를 괜히 돌린다"**는 실패 양상은 그대로 물려받습니다 — 정방향 보존 측정이 필수인 이유입니다.

### 남는 novelty 축 3개

**① 미분 가능 모듈 → reasoning 모델**
세 논문 모두 "픽셀 → 각도"를 뱉는 회귀/분류기입니다. 정방향 판정에는 **"이 물체는 원래 이렇게 서 있는 게 자연스럽다"는 semantic 지식**이 필요한데, 픽셀 회귀로는 그게 안 들어갑니다.
...
A-4는 "랜덤 회전 후 각도 복원"이라는 self-supervision을 씁니다. A-2는 데이터셋 prior를 씁니다. **우리는 본 모델을 직접 물어볼 수 있습니다** — 앞단이 보정한 뒤 **본 모델의 OCR 정확도**를 reward로. A-2가 "데이터셋의 방향 분포"라는 간접 지표로 우회한 걸, 우리는 본 모델에게 직접 물어서 하는 것이고, 이게 A-2의 identity prior를 MLLM으로 옮기는 가장 자연스러운 형태입니다.

**④ 이산 분류 → 임의 각도**
세 논문 모두 이산 회전군(A-1/A-2) 또는 360-way 분류(A-4)입니다. **임의 각도를 연속적으로 다루면서 순서·거리 정보를 유지하는 설계는 아무도 안 했습니다.**
...
- **A-4는 결과가 가장 강하지만**(augmentation 87% vs AMR 98%, 얼린 모델 transfer, 실물 촬영 검증) **feature tap 방식이라 아키텍처는 차용할 수 없습니다.** 우리는 **A-2형 독립 앞단**으로 갑니다.
...
- **셋 다 reasoning을 쓰지 않고, MLLM을 다루지 않고, 임의 각도를 다루지 않습니다.** 여기가 남은 자리입니다.
